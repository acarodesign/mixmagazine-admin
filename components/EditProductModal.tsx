...
const EditProductModal: React.FC<EditProductModalProps> = ({ product, onClose, onUpdate, showToast }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.name.trim() || !formData.price || !formData.price_card || !formData.stock) {
    ...
    const dataToUpdate: Product = {
        ...
        price_card: parseFloat(String(formData.price_card)),
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label htmlFor="edit-price" className="block text-sm font-medium text-slate-600 mb-1">Preço à Vista (un.)</label>
              ...
              <div>
                <label htmlFor="edit-price_card" className="block text-sm font-medium text-slate-600 mb-1">Preço Cartão (un.)</label>
                <input type="number" id="edit-price_card" name="price_card" value={formData.price_card} onChange={handleChange} required min="0" step="0.01" className="appearance-none relative block w-full px-4 py-3 border border-slate-300 bg-slate-100 placeholder-slate-400 text-slate-900 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:z-10 sm:text-sm transition-all duration-300" />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
